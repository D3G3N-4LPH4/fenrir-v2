...
class PriceQuote:
    ...
    def __repr__(self):
        return (
            f"PriceQuote(${self.price:.8f} from {self.source.value}, "
...
class AggregatedPrice:
    ...
    def get_best_quote(self) -> PriceQuote | None:
...
class PriceFeedManager:
    ...
    def __init__(self, birdeye_api_key: str | None = None, cache_ttl_seconds: int = 10):
    ...
    async def get_price(
        self, token_mint: str, force_refresh: bool = False
    ...
    async def _fetch_jupiter_price(self, token_mint: str) -> PriceQuote | None:
        """
    ...
    async def _fetch_birdeye_price(self, token_mint: str) -> PriceQuote | None:
        """
    ...
    async def _fetch_dexscreener_price(self, token_mint: str) -> PriceQuote | None:
        """
    ...
    async def start_price_monitoring(self, token_mints: list[str], interval_seconds: int = 5):
        """
    ...
    def get_cached_price(self, token_mint: str) -> AggregatedPrice | None:
...
class PriceAlertManager:
    ...
    async def add_alert(
        ...
        await self.price_feed.subscribe_to_price_updates(
            token_mint, lambda price: self._check_alerts(token_mint, price)
        )
...
async def example_usage():
    ...
    if price:
        ...
        for quote in price.quotes:
            print(
                f"   - {quote.source.value}: ${quote.price:.2f} "
