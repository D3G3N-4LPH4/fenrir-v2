...
class SMCAdapter:
    ...
    def __init__(
        ...
        self._emitted_structure_indices: set[int] = set()
    ...
    def get_claude_context(self) -> str:
        ...
        lines = [f"[SMC ANALYSIS @ price={price:.6f} | candles={self._candle_count}]"]
    ...
    def _compute_bos_choch(
        ...
        for i, row in struct_df.iterrows():
            ...
            if not pd.isna(choch_val) and choch_val != 0:
                ...
                self._structure.append(sig)
                if candle_idx not in self._emitted_structure_indices:
                    # Use negative index offset to differentiate CHoCH from BOS at same candle
                    choch_key = candle_idx + 10_000
                    if choch_key not in self._emitted_structure_indices:
                        self._emitted_structure_indices.add(choch_key)
                        self._event_queue.append(sig.to_dict())
